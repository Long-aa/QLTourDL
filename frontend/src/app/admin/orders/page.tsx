...
const STATUS_STYLES = {
  'Đã thanh toán': 'bg-blue-50 text-blue-600 border-blue-100',
  'Chờ xử lý': 'bg-cyan-50 text-cyan-600 border-cyan-100',
  'Đã hủy': 'bg-red-50 text-red-600 border-red-100',
  'Hoàn tiền': 'bg-slate-50 text-slate-600 border-slate-100'
};
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Danh sách Đơn hàng</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="py-4 px-6 text-[10px] font-bold text-gray-400 uppercase tracking-[0.1em]">Ngày đặt</th>
              ...
              {MOCK_ORDERS.map((order) => {
                ...
                return (
                  <tr key={order.id} className="group hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-[10px] font-bold text-gray-500 overflow-hidden border border-gray-100">
                    ...
                    <td className="py-4 px-6">
                      <span className="text-sm text-gray-600 font-medium">{order.tour}</span>
                    </td>
                    <td className="py-4 px-6">
        ...
        <div className="py-5 px-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/30">
          <p className="text-sm text-gray-500 font-medium">Hiển thị 1-10 của 124 đơn hàng</p>
          <div className="flex items-center gap-1.5">
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-600 hover:bg-white hover:shadow-sm text-xs font-bold transition-all border border-transparent hover:border-gray-100">2</button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-600 hover:bg-white hover:shadow-sm text-xs font-bold transition-all border border-transparent hover:border-gray-100">3</button>
            <span className="px-1 text-gray-400 text-xs">...</span>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-600 hover:bg-white hover:shadow-sm text-xs font-bold transition-all border border-transparent hover:border-gray-100">13</button>
