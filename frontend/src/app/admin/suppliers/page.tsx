    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Danh sách và thông tin đối tác cung cấp dịch vụ.</p>
        </div>
        <button 
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-1 items-center gap-3">
          ...
          <div className="relative">
            <select className="pl-10 pr-10 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-700 bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 cursor-pointer appearance-none transition-all min-w-[200px]">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                <th className="py-4 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Tên nhà cung cấp</th>
                <th className="py-4 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Loại dịch vụ</th>
                <th className="py-4 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Liên hệ</th>
                <th className="py-4 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Địa chỉ</th>
                <th className="py-4 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Trạng thái</th>
                <th className="py-4 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-wider text-right">Hành động</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {MOCK_SUPPLIERS.map((supplier) => (
                <tr key={supplier.id} className="group hover:bg-blue-50/30 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-blue-600 border border-gray-100 group-hover:bg-white transition-colors">
                        <supplier.icon className="w-5 h-5" />
                      </div>
                      <div>
                        <Link href={`/admin/suppliers/${supplier.id}`} className="font-bold text-gray-900 text-sm hover:text-blue-600 transition-colors">
                          {supplier.name}
                        </Link>
                        <p className="text-[10px] text-gray-500 font-medium mt-0.5 uppercase tracking-wider">Mã: {supplier.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                    <span className={`inline-flex px-3 py-1 rounded-lg text-xs font-bold ${supplier.typeColor}`}>
                      {supplier.type}
                    </span>
                  </td>
                  <td className="py-4 px-6">
                    <div className="text-xs space-y-1">
                      <p className="text-gray-600 font-medium">{supplier.contact.email}</p>
                      <p className="text-gray-400">{supplier.contact.phone}</p>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                    <span className="text-xs text-gray-600 font-medium leading-relaxed">{supplier.address}</span>
                  </td>
                  <td className="py-4 px-6">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold border ${
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full ${supplier.status === 'Đang hoạt động' ? 'bg-blue-600' : 'bg-red-600'}`}></span>
                      {supplier.status}
                    </span>
                  </td>
        ...
        <div className="py-5 px-6 border-t border-gray-100 flex items-center justify-between bg-gray-50/30">
          <p className="text-sm text-gray-500 font-medium">Hiển thị 1 đến 3 trong số 124 nhà cung cấp</p>
          <div className="flex items-center gap-1.5">
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-600 hover:bg-white text-xs font-bold transition-all border border-transparent hover:border-gray-100">12</button>
